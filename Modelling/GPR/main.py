...
def main():
    ...
    for ablated in ACTIVE_DESCRIPTORS:
        ...
        def loo_objective(**params):
            ...
            #weights["K"] = 0.0
            #weights["n"] = 0.0 # no hashtag if not wanted
    ...
    space = [
        Real(0, 5.0, name="circularity"),
        Real(0, 5.0, name="compactness"),
        Real(0, 5.0, name="K"),
        Real(0, 5.0, name="n"), # out
    ]

    @use_named_args(space)
    def objective(circularity, compactness, K, n): #circularity, compactness, K, n
        weights = {
            "diameter": FIXED_DIAMETER,        # FIXED_DIAMETER
            "circularity": circularity, #circularity
            "compactness": compactness, #put back compactness if wished
            "K": K,
            "n": n #put back n if wished
        }
    ...
    best_c, best_k, best_K, best_n = bo_result.x #best_c, best_k, best_K, best_n
    
    bo_weights = {
        "diameter": FIXED_DIAMETER,
        "circularity": best_c,
        "compactness": best_k, # best_k
        "K": best_K, #best_K or 0
        "n": best_n, # best_n
    }
    ...
    #print(f"Best BO weights: diameter = {FIXED_DIAMETER}, circularity = {best_c}, compactness = {best_k}, K = 0, n = 0")
    ...
    base_grid = {
        "diameter": [FIXED_DIAMETER], #[FIXED_DIAMETER]
        "circularity": publication_grid(W_MAX_GLOBAL), #publication_grid(W_MAX_GLOBAL)
        "compactness": publication_grid(W_MAX_GLOBAL), # publication_grid(W_MAX_GLOBAL) put back for 4D
        "K": publication_grid(W_MAX_GLOBAL), #publication_grid(W_MAX_GLOBAL)
        "n": publication_grid(W_MAX_GLOBAL), # put back for 4D publication_grid(W_MAX_GLOBAL) or [0.0]
    }
    ...
    # Train GP
    #gp = build_gp(random_state=RANDOM_STATE)
